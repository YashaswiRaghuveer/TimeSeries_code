...
load stocks.txt; stocks=stocks(:,3); y=dif(log(stocks))*400;
...
subplot(3,1,1)
plot(time,-cumirf2hat,'b-',time,-cumirf2a,'b--',time,-cumirf2b,'b:',time,zeros(size(time)),'k-'); axis([0 12 -15 20])
ylabel('Log Per Capita Stock Index')
title('Crude Oil Supply Shock')

[irf3hat,irf3a,irf3b,cumirf3hat,cumirf3a,cumirf3b]=stage2irf(y,q2q');
subplot(3,1,2)
plot(time,cumirf3hat,'b-',time,cumirf3a,'b--',time,cumirf3b,'b:',time,zeros(size(time)),'k-'); axis([0 12 -15 20])
ylabel('LPC Stock Index')
title('Aggregate Demand Shock')

[irf4hat,irf4a,irf4b,cumirf4hat,cumirf4a,cumirf4b]=stage2irf(y,q3q');
subplot(3,1,3)
plot(time,cumirf4hat,'b-',time,cumirf4a,'b--',time,cumirf4b,'b:',time,zeros(size(time)),'k-'); axis([0 12 -15 20])
ylabel('LPC Stock Index')
